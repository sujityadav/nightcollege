...
export async function GET(req, { params }) {
  await connectDB();
  const data = await Announcement.findById(params.id);
...
export async function PUT(req, { params }) {
  try {
    ...
    const data = await Announcement.findByIdAndUpdate(params.id, payload, { new: true, runValidators: true });
    return data ? NextResponse.json({ success: true, data }) : NextResponse.json({ success: false, message: "Not found" }, { status: 404 });
  } catch (error) {
...
export async function DELETE(req, { params }) {
  await connectDB();
  const data = await Announcement.findByIdAndDelete(params.id);
