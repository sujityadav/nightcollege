...
export async function POST(req) {
  try {
    ...
    const created = await SubAdministration.create({
      SubAdministrationData: body,
    });
    return NextResponse.json(
...
export async function GET(req) {
  try {
    ...
    const entries = await SubAdministration.find({
  "SubAdministrationData.data.administrationId": administrationId
}).sort({ createdAt: -1 });
