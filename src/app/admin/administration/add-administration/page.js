...
export default function AddEvents() {
  ...
  const fetchEventData = async (id) => {
    try {
      ...
      const res = await axios.get(`/api/administration/getbyId`, {
        params:{id:id},
        headers: {
      ...
      reset({
        title: event[0]?.AdministrationData?.data?.title,
        smallDescription: event[0]?.AdministrationData?.data?.smallDescription,
        largeDescription: event[0]?.AdministrationData?.data?.largeDescription,
      });
      setEditorContent(event[0]?.AdministrationData?.data?.largeDescription);
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      const url = comityId ? `/api/administration/${comityId}` : `/api/administration`;
    ...
    <div className="flex w-full">
      ...
      <div className='p-[20px] xl:p-[25px] 3xl:p-[1.563vw] w-full'>
        <h2 className='text-[#19212A] text-[22px] font-[700] mb-3'>
          {isUpdateMode ? 'Update Administration' : 'Add Administration'}
        </h2>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='bg-white card-shadow p-[25px]'>
          <div className='px-[250px] space-y-3'>
            ...
            <div className='mt-6 flex justify-center gap-6'>
              <Link href="/admin/administration" className='cancelbtn px-4 py-2'>Cancel</Link>
              <Button
                type='submit'
