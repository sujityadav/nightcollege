...
export default function AddEvents() {
  ...
  const administrationId = searchParams.get('id'); // <-- read id from query
  ...
  useEffect(() => {
    if (administrationId) {
      ...
      fetchDepartmentsData(administrationId);
    }
  }, [administrationId]);
  ...
  const fetchDepartmentsData = async (id) => {
    try {
      ...
      const res = await axios.get(`/api/subadministration/getbyId`, {
        params:{id:id},
        headers: {
      ...
      reset({
        title: event[0]?.SubAdministrationData?.data?.title,
        smallDescription: event[0]?.SubAdministrationData?.data?.smallDescription,
        largeDescription: event[0]?.SubAdministrationData?.data?.largeDescription,
      });
      setEditorContent(event[0]?.SubAdministrationData?.data?.largeDescription);
  ...
  const onSubmit = async (formData) => {
    ...
    formData.administrationId = searchParams.get('administrationId');

    const payload = {
      data: formData,
      ...(administrationId && { _id: administrationId }),
    };

    try {
      const url = administrationId ? `/api/subadministration/${administrationId}` : `/api/subadministration`;
      const method = administrationId ? 'put' : 'post';
     ...
     console.log("response", response)
      if (response?.data?.success) {
        toast.current.show({
          severity: 'success',
          summary: administrationId ? 'Updated' : 'Saved',
          detail: `Content ${administrationId ? 'updated' : 'saved'} successfully ✅`,
          life: 3000,
        });
        router.push(`/admin/sub-administration?administrationId=${searchParams.get('administrationId')}`);
      }
    } catch (err) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='bg-white card-shadow p-[25px]'>
          <div className='px-[250px] space-y-3'>
            ...
            <div className='mt-6 flex justify-center gap-6'>
              <Link href={`/admin/sub-administration?administrationId=${searchParams.get('administrationId')}`} className='cancelbtn px-4 py-2'>Cancel</Link>
              <Button
                type='submit'
