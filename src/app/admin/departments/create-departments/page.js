...
export default function AddEvents() {
  ...
  const fetchDepartmentsData = async (id) => {
    try {
      ...
      reset({
        title: event[0]?.InnerDepartmentsData?.data?.title,
        smallDescription: event[0]?.InnerDepartmentsData?.data?.smallDescription,
        largeDescription: event[0]?.InnerDepartmentsData?.data?.largeDescription,
      });
      setEditorContent(event[0]?.InnerDepartmentsData?.data?.largeDescription);
  ...
  const onSubmit = async (formData) => {
    ...
    formData.depatmentId = searchParams.get('depatmentId');
    ...
    try {
      const url = comityId ? `/api/innerdepartments/${comityId}` : `/api/innerdepartments`;
      ...
      if (response?.data?.success) {
        ...
        router.push(`/admin/departments?depatmentId=${searchParams.get('depatmentId')}`);
      }
    } catch (err) {
